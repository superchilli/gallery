...
class Follow(db.Model):
    ...
    followed_id = db.Column(db.Integer, db.ForeignKey('users.id'),
                            primary_key=True)
    timestamp = db.Column(db.DateTime, defaule=datetime.utcnow())
...
class User(UserMixin, db.Model):
    ...
    def __index__(self, **kwargs):
        ...
        if self.role is None:
            if self.email == current_app.config['GALLERY_ADMIN']:
                self.role=Role.query.filter_by(permissions=0xff).first()
            if self.role is None:
                self.role=Role.query.filter_by(default=True).first()
        if self.eamil is not None and self.avatar_hash is None:
            self.avatar_hash=hashlib.md5(
                self.email.encode('utf-8')).hedigest()
    ...
    def gravatar(self, size=100, default='identicon', rating='g'):
        if request.is_secure:
            ...
            url='http://secure.gavatar.com/avatar'
...
login_manager.anonymours_user = AnonymousUser
